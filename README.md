...
[![Status](https://img.shields.io/badge/Status-v0.2.0-brightgreen)](https://github.com/N1ne-W/AutoMeme)
...
- 🙌 **8 种手势识别** — OMG / NFB / Donk / MonkeyThink / ThumbsUp + 3 种通用手势
- 🎤 **离线语音触发** — Vosk 中英文关键词识别，支持自定义关键词绑定表情包
- 🧠 **智能判定引擎** — 状态机 + 时间防抖（GestureStabilizer）+ 动态冷却 + 优先级仲裁
- 🔊 **音频子系统** — 独立音频状态机，fade-in/fade-out，owner 安全校验，可配置生命周期
- 🎨 **PyGame 实时渲染** — 表情包 + 音频同步淡入淡出，MediaPipe 骨骼线 Debug
- 🩺 **诊断体系** — trace_id 端到端追踪，9 段链路诊断事件，CLI debug 工具
...
> 通过 config/mappings.default.json 或 config/mappings.user.json 自定义绑定
...
| 语音识别 | Vosk 离线（中英文）                 |
| 音频播放 | PyGame mixer (fade/loop/owner)      |
| GUI 渲染 | PyGame 无边框透明叠加层             |
| 打包分发 | PyInstaller / NSIS                  |
| 配置格式 | JSON + YAML                         |
| 测试     | pytest (85 tests)                   |
| 诊断     | trace_id + log_trace + Throttle     |

## 🗺️ Roadmap

- [x] Phase 0 — 文档体系 + 项目初始化
- [x] Phase 1 — 模块化视觉管道 + 8 个特征检测器
- [x] Phase 2 — 判定引擎（状态机 + 映射 + GestureStabilizer）
- [x] Phase 3 — Vosk 语音关键词触发 + AudioSlot 音频子系统
- [x] Phase 4 — 无边框透明叠加层 + fade-in/fade-out + GIF
- [ ] Phase 5 — GUI 组合编辑器
- [ ] Phase 6 — Windows 安装程序

### 已完成增强 (v0.2.0)

- ✅ Debug 诊断体系（9 段链路 trace）
- ✅ 白盒审计 9 项 P0/P1 修复
- ✅ 动态冷却时长 + 词边界关键词匹配
- ✅ 空 landmarks 防护 + cv2 顶层导入
- ✅ Throttle 内存上限 + MappingEngine 公共 API
